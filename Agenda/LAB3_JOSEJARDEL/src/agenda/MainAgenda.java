...
public class MainAgenda {
	//private LimparBuffer limparBuffer;
	...
	private static void listaContatos(Agenda agenda) {
		...
		for (int i = 0; i < agenda.getContatos().length; i++) {
			if (agenda.getContatos()[i] != null) {
				System.out.println(formataContato(i + 1, agenda.getContatos()[i]));
			}
		}
	}
	...
	private static void listaFavoritos(Agenda agenda) {
		...
		for (int i = 0; i < agenda.getFavoritos().length; i++) {
			if (agenda.getFavoritos()[i] != null) {
				System.out.println(formataContato(i + 1, agenda.getFavoritos()[i]));
			}
		}
	}
	...
	private static void cadastraContato(Agenda agenda, Scanner scanner) {
		...
		if (agenda.validaPosicao(posicao, agenda.getContatos().length)) {
			System.out.print("\nNome> ");
	...
	private static void adicionaFavorito(Agenda agenda, Scanner scanner) {
		System.out.println("/nContato> ");
		int idContato = scanner.nextInt();
		
		
		if (agenda.verificarSeExisteContato(idContato, agenda.getContatos())){
			System.out.println("\nPosição>");
			int posicao = scanner.nextInt();
			if (agenda.validaPosicao(posicao, agenda.getFavoritos().length)) {
				System.out.println(agenda.adcionaFavorito(idContato, posicao));
			}
			else {
				System.out.println("POSIÇÃO INVÁLIDA");
			}
			...
			System.out.println("NÃO HÁ CONTATO CADASTRADO NESSA POSIÇÃO");
		}
	}
	
	private static void removeFavorito(Agenda agenda, Scanner scanner) {
		System.out.println("\nPosição na lista de favoritos> ");
		int posicao = scanner.nextInt();
		if (agenda.validaPosicao(posicao, agenda.getFavoritos().length)) {
			System.out.println(agenda.removeFavorito(posicao));
